    ...
    const bool separate_into_words)
{
    ...
    if (char_start_key != str.end())
    {
        ...
        string::const_iterator char_start_entry = find_if(char_end_key, str.end(), is_not_space);
        if (separate_into_words)
        {
            // populate map one word at a time
            while (char_start_entry != str.end())
            {
                // identify next word: [char_start_entry, char_end_entry)
                string::const_iterator char_end_entry = find_if(char_start_entry, str.end(), is_space);

                // populate corresponding entry
                result[first_word].push_back(string(char_start_entry, char_end_entry));

                // continue scanning the string
                char_start_entry = find_if(char_end_entry, str.end(), is_not_space);
            }
        }
        else
        {
